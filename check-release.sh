#!/usr/bin/env bash
# Woodlands Galaxy Night-Sky — pre-flight release check.
...
PAGES=(Woodland_Galaxy.html telescope-types.html moon-tonight.html tracking-methods.html tonights-tour.html sky-finder.html eyepiece-fov.html kiosk.html align-stars.html plan-ahead.html solar.html object-cards.html checklist.html constellation-story.html scale-model.html highlights-card.html qr-poster.html identify.html audio-tour.html almanac.html quiz.html occultations.html sky-guide.html)
# Pages/assets the service worker must cache for offline use.
CACHED=(Woodland_Galaxy.html telescope-types.html moon-tonight.html tracking-methods.html tonights-tour.html sky-finder.html eyepiece-fov.html kiosk.html align-stars.html plan-ahead.html solar.html object-cards.html checklist.html constellation-story.html scale-model.html highlights-card.html qr-poster.html identify.html audio-tour.html almanac.html quiz.html occultations.html sky-guide.html site-config.js astro-core.js version.js)
...
if grep -alr 'Woodlands Galaxy Night-Sky <b>v' --include='*.html' . | grep -v '/archive/' | grep -q .; then
  grep -anr 'Woodlands Galaxy Night-Sky <b>v' --include='*.html' . | grep -v '/archive/' | sed 's/^/    /'
...
CACHE=$(grep -aoE "wg-sky-v[0-9]+" sw.js | head -n 1)
if [ -n "$CACHE" ]; then ok "cache name is $CACHE"; else bad "no wg-sky-vN cache name in sw.js"; fi
for a in "${CACHED[@]}"; do
