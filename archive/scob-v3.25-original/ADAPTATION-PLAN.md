...
- **Coordinates** — 2 constants: `scob-dashboard-v3.html` line 760 and `astro-core.js` line 9.
...
Re-point coordinates, swap NEA region to North, rebrand titles/manifest/footer, set session to 7:30–9:30, update the fleet reference and the run-sheet instrument slots, fix external links. Regenerate `test-astro.js` reference values and confirm they pass. Deliverable: a working `woodlands-dashboard.html` (equivalent of `scob-dashboard-v3.html`).
