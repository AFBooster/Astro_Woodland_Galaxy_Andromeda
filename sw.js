/* Woodlands Galaxy Night-Sky — service worker: offline cache so the installed app opens with no signal */
const CACHE = 'wg-sky-v2';
const ASSETS = [
  'Woodland_Galaxy.html',
  'sky-guide.html',
  ...
  if (isPage) {
    e.respondWith(
      fetch(e.request).then(res => {
        ...
        return res;
      }).catch(() => caches.match(e.request).then(hit => hit || caches.match('Woodland_Galaxy.html')))
    );
    return;
  }
  e.respondWith(
    caches.match(e.request).then(hit => hit || fetch(e.request).then(res => {
      ...
      caches.open(CACHE).then(c => c.put(e.request, copy)).catch(() => {});
      return res;
    }).catch(() => caches.match('Woodland_Galaxy.html')))
  );
});
